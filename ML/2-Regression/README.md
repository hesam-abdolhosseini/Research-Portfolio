...
Regression is used when the **target variable is continuous**, such as predicting CO₂ emissions, house prices, or medical measurements.  
...
این ماژول به **رگرسیون** در یادگیری ماشین می‌پردازد؛ روشی برای پیش‌بینی مقادیر پیوسته مانند انتشار CO₂، قیمت خانه یا فشار خون.

### **مباحث پوشش داده‌شده**
- رگرسیون خطی ساده  
- رگرسیون خطی چندگانه  
- معیارهای ارزیابی مدل (MAE، MSE، RMSE، R² و …)  
- آماده‌سازی داده و تقسیم‌بندی آموزش/آزمون  

### **ساختار پوشه**
- پوشه `linear_regression` شامل آزمایش کامل رگرسیون ساده و دیتاست  
- پوشه `multiple_linear_regression` شامل آزمایش کامل رگرسیون خطی چندگانه و دیتاست  
- پوشه‌های آینده شامل رگرسیون غیرخطی و چندجمله‌ای خواهند بود  

هدف اصلی این ماژول، **شفافیت، بازتولیدپذیری و آموزش مرحله‌به‌مرحله** است.