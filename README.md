# Research-Portfolio

Portfolio highlighting research in optimization, machine learning, and secure systems.  
Emphasis on reproducibility, efficiency, and practical prototypes for resource-constrained environments.

---

## 📂 Structure
- **optimization/** → Multi-objective trade-offs in latency, reliability, and buffer constraints  
- **ml/** → Step-by-step machine learning projects (IBM ML0101EN aligned), with bilingual notes and figures  
- **Cybersecurity_Fundamentals/** → Consolidated notes from ISC2 CC and CompTIA Security+
- **teaching-guides/** → Bilingual guides (currently: Jupyter Notebook on Ubuntu 24.04 and Windows) 

---

## 🔗 Quick Links
- [C-Programming-Portfolio](https://github.com/hesam-abdolhosseini/C-Programming-Portfolio)

---

## 📝 Notes
Each module will include:
- README with objectives, methods, and results  
- Small data samples for reproducibility  
- One-command scripts for replication  
- Minimal tests to validate functionality